...
function options() {
	...
	exports.init = function(config) {
		...
		// set the token
		this.agent.token(auth)
		
		// emit events
		this.talkOn('ready', function() {
			// epg.agent.status();
		});
		
		this.talkOn('lineupMap', function(listings) {
			//console.log('lineupMap', util.inspect(listings, {showHidden: false, depth: null}));
		});
	...
	/**
	 * Emitter events for server and socket
	 * 
	 * */
	...
	exports.talkOn = function(listenFor, run) {
		if(this.io) {
			this.io.on(listenFor, run);
		}
		this.on(listenFor, run);
	}
	exports.talkOnce = function(listenFor, run) {
		if(this.io) {
			this.io.once(listenFor, run);
		}
		this.once(listenFor, run);
	}
	exports.talkRemove = function(event, remove) {
		if(this.io) {
