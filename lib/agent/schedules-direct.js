...
var debug = require('debug')('epg:lib:agent:schedules-direct');
...
_.extend(Agent.prototype, {
	...
	programs: function(data, callback) {
		...
		request
			...
			.end((err, res) => {
				...
				if(err && err.response.body.code !== 4102) {
					epg.talk(['programsError', data.iden],{action: 'programs', error:  err.response.body }); 
				} else if(err && err.response.body.code === 4102) {
	...
	metadata: function(data) {
		return new Promise( ( resolve, reject ) => {
			...
			let auth = epg.get('auth');
			if(_.isEmpty(auth.token)) {
				epg.talk(['metadataError', data.iden],{action: 'metadata', error: { message: 'No auth token present'}});
				return
			} 
			
			if( !data.programID ) {
				epg.talk(['celebrityError', data.iden],{action: 'celebrity', error: { message: 'No programs present'}});
				debug(data)
				return reject('No programs present');
			} 
			let programs = data.programID
			if ( !Array.isArray(programs) ) {
				programs = [ programs ];
			}
			
			//debug('agent metadata');
			...
			request
				...
				.end((err, res) => {
					...
					if ( err ) {
						reject(403);
						epg.talk(['metadataError', data.iden],{action: 'metadata', error:  err }); 
					} else if(res.status === 403) {
						reject(403);
						epg.talk(['metadataError', data.iden],{action: 'metadata', error:  err.response.body }); 
					} else if(err && err.response.body.code !== 4102) {
						reject(4102);
						epg.talk(['metadataError', data.iden],{action: 'metadata', error:  err.response.body }); 
					} else if(err && err.response.body.code === 4102) {
						resolve([]);
						epg.talk(['metadata', data.iden], []);
					} else if(res.status === 200) {
						//debug(res.body);
						resolve(res.body);
						epg.talk(['metadata', data.iden], res.body);
					} else {
						resolve([]);
						debug(res.status,res.body)
					}
	...
	celebrity: function( data ) {
		return new Promise( ( resolve, reject ) => {
			...
			let auth = epg.get('auth');
			if(_.isEmpty(auth.token)) {
				epg.talk(['celebrityError', data.iden],{action: 'celebrity', error: { message: 'No auth token present'}});
				return reject('No auth token present')
			} 
			
			if( !data.personId ) {
				epg.talk(['celebrityError', data.iden],{action: 'celebrity', error: { message: 'No personId present'}});
				return reject('No nameId present');
			} 
			
			//debug('agent celebrity metadata', data.personId);
			
			request
				.get('https://json.schedulesdirect.org/20141201/metadata/celebrity/' + data.personId)
				.set('token', auth.token)
				.set('User-Agent', this.agent)
				.end((err, res) => {
					//debug( res.status )
					...
					if(res.status === 403) {
						reject(403);
						epg.talk(['celebrityError', data.iden],{action: 'celebrity', error:  err.response.body }); 
					} else if(err && err.response.body.code !== 4102) {
						reject(4102);
						epg.talk(['celebrityError', data.iden],{action: 'celebrity', error:  err.response.body }); 
					} else if(err && err.response.body.code === 4102) {
						debug('no data');
						resolve([]);
						epg.talk(['celebrity', data.iden], []);
					} else if(res.status === 200) {
						//debug('got data for celebrity ', data.name);
						resolve(res.body);
						epg.talk(['celebrity', data.iden], res.body);
					} else {
						reject(res.status);
						debug(res.status,res.body)
					}
