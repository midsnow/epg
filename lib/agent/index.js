			...
			request
				...
				.end((err, res) => {
					...
					if(err) {
						...
						epg.talk(['statusError', data.iden],{action: 'status', error:  err.response.body, server: err.response.body}); 
					} else if(res.status === 200) {
						//debug(res.body);
	...
	headends: function(data, callback ) {
		...
		request
			...
			.end((err, res) => {
				// Calling the end function will send the request
				if(err) {
					epg.talk(['headendsError', data.iden], {action: 'headends', error:  err.response.body, server: err.response.body});
					return;
				} else if(res.status === 200) {
					//debug(res.body);
					// create the return 
					if(_.isArray(res.body)) {
						...
						epg.talk(['headendsError', data.iden], {action: 'headends', error: { message: 'Error talking to agent'}, server: err.response.body});
					}
	...
	lineupAdd: function(data, callback) {
		...
		request
			...
			.end((err, res) => {
				...
				debug(err, res);
				if(err) {
					epg.talk(['lineupAddError', data.iden], {action: 'lineupAdd', error: err.response.body}); 
				} else if(res.status === 200) {
					//debug(res.body);
		...
		request
			...
			.end((err, res) => {
				...
				if(res.status === 200) {
					...
					epg.talk(['lineupRemoveError', data.iden], {action: 'lineupRemove', error:  err.response.body, server: err.response.body}); 
				}
	...
	lineupMap: function(data, callback) {
		...
		request
			...
			.end((err, res) => {
				// Calling the end function will send the request
				if(err) {
					epg.talk(['lineupMapError', data.iden],{action: 'lineupMap', error:  err.response.body, server: err.response.body}); 
				} else if(res.status === 200) {
					//debug(res.body);
					// normalize the data
		...
		request
			...
			.end((err, res) => {
				...
				if(err && err.response.body.code !== 4102) {
					epg.talk(['lineupsError', data.iden],{action: 'lineups', error:  err.response.body, server: err.response.body }); 
	...
	schedules: function(data, callback) {
		...
		request
			...
			.end((err, res) => {
				// Calling the end function will send the request
				if(err && err.response.body.code !== 4102) {
					epg.talk(['schedulesError', data.iden],{action: 'schedules', error:  err.response.body, server: err.response.body }); 
				} else if(err && err.response.body.code === 4102) {
	...
	programs: function(data, callback) {
		...
		request
			...
			.end((err, res) => {
				// Calling the end function will send the request
				if(err && err.response.body.code !== 4102) {
					epg.talk(['programsError', data.iden],{action: 'programs', error:  err.response.body, server: err.response.body }); 
				} else if(err && err.response.body.code === 4102) {
