...
exports = module.exports = function models() {
	...
	if ( db.driver = 'mysql' ) {
		db.driver = path.join( '/', epg._options.moduleRoot, 'lib', 'adapter', 'caminte-mysql.js' );
	}
	
	var schema  = new Schema( db.driver, db, ( err ) => {
		if ( err ) {
			debug('Error Caminte', e );
			epg._connected.db = false;
			return;
		} else {
			if ( schema.client.state == 'disconnected' ) {
				debug( 'Connection', schema.client.state );
				epg._connected.db = false;
				return;
			}
			run.call( this );
	...
	function run() {
		...
		*/
		...
		var Stations = schema.define('Stations', {
			...
			stationLogo: { type: schema.JSON, "null": false },
		...
		var Schedules = schema.define('Schedules', {
			...
			image: { type: schema.JSON, default: {}, "null": false   },
			images: { type: schema.JSON, default: {}, "null": false   },
		});
		...
		var Person = schema.define('Person', {
			nameId: { type: schema.Number, index: true },
			personId: { type: schema.Number, index: true },	
			name: { type: schema.String},
			photo: { type: schema.JSON },	
			images: { type: schema.JSON, default: [] },
		});
		
		epg.Models = schema.models;
		epg.Schema = schema;
		epg._connected.db = true;
		
		schema.adapter.autoupdate(function( err ) {
			debug('end db', err);
		})
	}
