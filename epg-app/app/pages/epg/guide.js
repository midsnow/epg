...
let LEFT_COLOR_FROM = hexToRgb("#471061");
let LEFT_COLOR_TO = hexToRgb("#1C1061");
let TOP_COLOR_FROM = hexToRgb("#471061");
let TOP_COLOR_TO = hexToRgb("#1C1061");
let LC = mixColors(
	LEFT_COLOR_FROM,
	LEFT_COLOR_TO,
	0
...
export default class EPGHome extends PureComponent { 
	...
	componentDidMount() {
		...
		// grab the channels so they load while we wait on guide data
		this._channelList( { 
			...this.props, 
			...this.state 
		} , ( err, ret ) => {
	...
	_clean() {
		if ( this._mounted.length > 0 ) {
			this._mounted.forEach( ref => {
	...
	componentWillReceiveProps ( props ) {
		...
		if ( props.routes[2].path === 'guide' && props.routes[2].path !== this.state.route ) {
			debug( 'get guideData' );
			this.setState( { 
					entries: [], 
					guideLoaded: false, 
	...
	getGuideData ( id, start, end ) {
		...
		.then(data => {
			...
			this.setState({
				...
				debug('start CHANNEL LIST');
				this._channelList({ 
					...this.props, 
					...this.state 
				}, ( err, ret ) => {
					debug('goit CHANNEL LIST', ret);
					this._update = true;
					this.setState( {  
						channelList: ret,
						guideLoaded: true, 
	...
	/** ****************RENDER************************************* **/
	render ( ) {		
		LEFT_COLOR_FROM = hexToRgb(this.props.theme.baseTheme.palette.canvasColor);
		LEFT_COLOR_TO = hexToRgb(ColorMe(10, this.props.theme.baseTheme.palette.canvasColor).bgcolor);
		TOP_COLOR_FROM = hexToRgb(this.props.theme.baseTheme.palette.primary1Color);
		TOP_COLOR_TO = hexToRgb(ColorMe(10, this.props.theme.baseTheme.palette.primary1Color).bgcolor);
		
		debug(LEFT_COLOR_FROM, LEFT_COLOR_TO, TOP_COLOR_FROM, TOP_COLOR_TO);
	...
	renderChannel ( ) {
		...
		return (<RenderChannel 
			ref={ ref => this._mounted.push(ref) } 
			renderChannel={ channel } 
			setTimer={ this.setTimer }  
			deleteTimer={ this.deleteTimer } 
			deleteSeries={ this.deleteSeries.bind(this) }  
			bgcolor={ rgbToHex( { ...LC } ) } 
			goBack={()=>(this.props.goTo({ path: '/tv/guide' }) )} 
			{ ...this.props } 
		/>);
	}
	...
	renderGuide ( ) { 
		...
		return (
			<div ref={ ref => this._mounted.push(ref) }  style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: `rgb(${TC.r},${TC.g},${TC.b})`, zIndex: 1000 }} >
				{changeChannel}
			...
			if ( Object.keys(props.groups).length > 0 && ( props.group !== this.state.group || props.sortGuideBy !== this.state.sortGuideBy || this.state.channelList.length === 0 ) ) {
					let a = props.groups[props.group]
						...
						.map( channel2 => {
								...
								let data = this.state.entries[channel];
	...
	_renderBodyCell( { index, isScrolling, key, style } ) {
		...
		const classNames = {
			  ...
			  padding: '0 5',
			  border: "4px solid "  +  ColorMe( -5, rgbToHex(LC) ).bgcolor,
			  ...style,
		}; 
		...
		return  (
			...
			>
				...
				<div style={{ maxHeight: style.height / 2, fontWeight: 400,  width: '100%', overflow: 'hidden', padding: ' 0', }} >{data.episode || data.plotOutline} </div>
...
EPGHome.getInitialData = function ( params ) {
